...
public class InfluenceMap implements Serializable {
	...
	/*
	 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	 * +++++++++++++++++++++ ++++++++++++++++++++++++++++++Izvadeni uslovite od
	 * HIT+++++++++++++++++++++++++++++++++++++++++
	 * ++++++++++++++++++++++++++++++
	 * +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	 */
	private void southernIsNotAHit(int i, int j) {
	...
	private void nothernIsNotAHit(int i, int j) {
	...
	private void easternIsNotAHit(int i, int j) {
	...
	private void westernIsNotAHit(int i, int j) {
	...
	public void sunk(int i, int j) {
		if (map[i][j] == hit) {
			...
			// if hit is on left collumn but not top left or bottom lef
	 ...
	 * +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	 */
	private void hitIsNotOnAnEdge(int i, int j) {
		// if the hit is not on an edge
		if (i < 9 && i > 0 && j < 9 && j > 0) {
	...
	private void hitIsOnLeftCollumnButNotTopOrBottomLeft(int i, int j) {
	...
	private void hitIsOnRightCollumnButNotTopOrBottomRight(int i, int j) {
		// if hit is on right most column but not top right or bottom right
		if (j == 9 && i != 0 && i != 9) {
			// dec to left if even
			if (map[i][j - 1] != hit && map[i][j - 1] % 2 == 0) {
	...
	private void hitOnBottomRow(int i, int j) {
		// if hit on bottom row
		if (i == 9) {
			// dec above if even
			if (map[i - 1][j] != hit && map[i - 1][j] % 2 == 0) {
	...
	private void hitOnTopRow(int i, int j) {
		// if hit is on top row
		if (i == 0) {
			// dec below if even
			if (map[i + 1][j] != hit && map[i + 1][j] % 2 == 0) {
	...
	public void miss(int i, int j) {
		...
		try { // if i+1 is a hit and i+2 is a hit and north, west, east are all
			 ...
			 * 0< OX0 X inc
			 */
			if (map[i + 1][j] == hit /* below is a hit */&& map[i + 2][j] == hit /*
																				 * i
																				 * +
																				 * 2
																				 * is
	...
	/**
	 * Return the i or j of the cell with highest influence value
	 * 
	 * @param a
	 *            moze da primi vrednost i ili j
	 */
	public int getHotSpot(char a) {
		...
		return 0; // Vo slucaj da vnese pogreshna bukva
	}
